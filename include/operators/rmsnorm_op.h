...
class RmsNormOp {
 public:
  ...
  RmsNormOp(runtime::DeviceTensorView<const float> gamma, float epsilon);
  ...
  runtime::DeviceTensorView<const float> gamma() const;
  float epsilon() const;

 private:
  runtime::DeviceTensorView<const float> gamma_;
  float epsilon_;
};
